...
import { getFluency, updateFluency, calculateTestScore } from '../fluency'
...
function generateProblems(digits: number[], count: number): Problem[] {
  const pool: Problem[] = []
  ...
  pool.sort((a, b) => {
    const sa = getFluency(a.a, a.b)
    const sb = getFluency(b.a, b.b)
...
function AdditionExercise() {
  ...
  const [problems, setProblems] = useState<Problem[]>(() => generateProblems(digits, count))
  ...
  const expected = problem ? problem.a + problem.b : 0
  ...
  const pressDigit = useCallback((d: number) => {
    ...
    if (numAnswer === expected) {
      ...
      updateFluency(problems[currentIndex].a, problems[currentIndex].b, testScore)
  ...
  }, [answer, showCorrect, done, expected, problems, currentIndex, questionStart, hadWrong])
  ...
  const restart = () => {
    setProblems(generateProblems(digits, count))
    setCurrentIndex(0)
    setAnswer('')
    setCorrectCount(0)
    setShowCorrect(false)
    ...
    return (
      <div>
        <div className="top-bar">
          <Link href="/addition" className="btn btn-back">Back</Link>
        </div>
    ...
    <div>
      ...
      <div className="exercise-screen">
        ...
        <div className="exercise-prompt">
          {problem!.a} + {problem!.b} = <span className={`exercise-answer ${answerColor}`}>{answer || ' '}</span>
        </div>
